...
#define Reward_Threshold_Proposed_Phase 50
...
#define T0 10    //Need For Certainty
...
//*********************************************************************************************************//
void Init_List_Of_Agent_Slice(int List_Of_Slice_Ag[], int Num_Of_Agent_Slice, int Possible_Slice_List[]) {
    int ItemIndex, i;
...
int Select_Cell(int List[], float Q[][Col], int Slice) {
    ...
    if (Num_Of_Zero == Col) {
        ItemIndex = rand() % (Num_Of_Cells - 0); //No one of Action be selected
    ...
    /************************************************************************************/
        ...
        if (IS_There_Up_threshold(Q, Slice) == 1) {
            ...
            TypeOfCells = 2; // Denote That There IS some Cells Greater than threshold
        }
        //*******************************************************************//
        ...
        //*******************************************************************//   	
            ...
            //***********************************************************************//
            //*there are some items that greater than zero and lower than threshold*//
            //***********************************************************************//  		    					  				                  						        	
            else {
                ItemIndex = rand() % (Num_Of_Cells - 0);
                TypeOfCells = 4; //Denote That There Some Cells That  Greater than Zero and Lower Than threshold
            }
        }
    }
    /************************************************************************************/
    ...
    switch (TypeOfCells) {
        case 1: cout << "\nAll Of Cells Are Zero";
            break;
        case 2: cout << "\nThere Are Some Cells Greater Than threshold";
            break;
        case 3: cout << "\nThere Are Some Cells that not Verified Yet";
            break;
        case 4: cout << "\nThere All Cells That Greater than Zero and Lower Than threshold";
            break;
    }
 ...
 *														*
 ********************************************************/
float Environment_Feedback(int Cell1, int Slice1, int Cell2, int Slice2, int Cell3, int Slice3) {
    int R = 0;
...
//---------------------------------------------------------------------------------------------------------//
void Critic_Type_1_Without_Ag1(float Reward, float RList[]) {
    RList[0] = 0;
...
//---------------------------------------------------------------------------------------------------------//
void Critic_Type_1_Without_Ag2(float Reward, float RList[]) {
    RList[1] = 0;
...
//---------------------------------------------------------------------------------------------------------//
void Critic_Type_1_Without_Ag3(float Reward, float RList[]) {
    RList[2] = 0;
...
//---------------------------------------------------------------------------------------------------------//
void Critic_Type_1_Without_Ag1_Ag2(float Reward, float RList[]) {
    RList[0] = 0;
    RList[1] = 0;
...
//---------------------------------------------------------------------------------------------------------//
void Critic_Type_1_Without_Ag1_Ag3(float Reward, float RList[]) {
    RList[0] = 0;
    RList[2] = 0;
...
//---------------------------------------------------------------------------------------------------------//
void Critic_Type_1_Without_Ag2_Ag3(float Reward, float RList[]) {
    RList[1] = 0;
    RList[2] = 0;
...
int main() {
    ...
    for (Episode_Counter = 0; Episode_Counter < Episode_Number_Train; Episode_Counter++) {
        ...
        while (MAS_Reward_Episode < Reward_Threshold) {
            NR_Ag1 = 0;
            NP_Ag1 = 0;
            NR_Ag2 = 0;
            NP_Ag2 = 0;
            ...
            *************************************************************************/
            ...
            for (i = 0; i < Num_Of_Select; i++) //**//***//
            {
                ...
                MAS_Reward_Train = Environment_Feedback(Slice1, Cell1, Slice2, Cell2, Slice3, Cell3);
...
for(i=0;i<Num_Of_Select;i++) //**/ /***
    ...
    //        for( i=1;i<=100;i++)
    {
        ...
        NumOfRewarding = 0;
        while (MAS_Reward_Episode < Reward_Threshold_Proposed_Phase) {
            /************************************************************************
             *																	    *
            ...
            for (i = 0; i < Num_Of_Select; i++) // Start of Any Round	(Any Round Includes 3 Tries)
            {
                ...
                if (i == 0) {
                    ...
                    if ((LearningRate(Q2, 2) >= 9) && (LearningRate(Q3, 3) < 9))
                        MAS_Reward_Test += Envionment_Feedback_Agent1(Slice1, Cell1) + 10;
                    if ((LearningRate(Q2, 2) < 9) && (LearningRate(Q3, 3) >= 9))
                        MAS_Reward_Test += Envionment_Feedback_Agent1(Slice1, Cell1) + 5;
                    if ((LearningRate(Q2, 2) >= 9) && (LearningRate(Q3, 3) >= 9))
                        MAS_Reward_Test += Envionment_Feedback_Agent1(Slice1, Cell1) + 15;
                    ...
                    if (abs(R1 - RealRewardAg1) <= CorrThreshold) CorrAg1++;
                    else CorrAg1--;
                    if (abs(R2 - RealRewardAg2) <= CorrThreshold) CorrAg2++;
                    else CorrAg2--;
                    if (abs(R3 - RealRewardAg3) <= CorrThreshold) CorrAg3++;
                    else CorrAg3--;
                 ...
                 ************************************************************************/
                    ...
                    if (R2 >= TS2) {
                        ...
                        if ((LearningRate(Q2, 2) < 9) && (LearningRate(Q3, 3) < 9))
                            MAS_Reward_Test += Envionment_Feedback_Agent2(Slice2, Cell2);
                        if ((LearningRate(Q2, 2) < 9) && (LearningRate(Q3, 3) >= 9))
                            MAS_Reward_Test += Envionment_Feedback_Agent2(Slice2, Cell2) + 5;
                        // MAS_Reward_Episode+=MAS_Reward_Test;	
                    }
                    if (R3 >= TS3) {
                        if ((LearningRate(Q3, 3) < 9) && (LearningRate(Q2, 2) < 9))
                            MAS_Reward_Test += Envionment_Feedback_Agent3(Slice3, Cell3);
                        if ((LearningRate(Q3, 3) < 9) && (LearningRate(Q2, 2) >= 9))
                            MAS_Reward_Test += Envionment_Feedback_Agent3(Slice3, Cell3) + 10;
                        // MAS_Reward_Episode+=MAS_Reward_Test;	
                    }
    ...
    while ((LearningRate(Q2, 2) < 9) && (Episode_Number_Ag2 + Episode_Number_Ag1 <= Iteration))
    //Start Of Episode in Test Phase
    //for(int Iteration=1;Iteration<=100;Iteration++)
    {
        ...
        while (MAS_Reward_Episode < Reward_Threshold_Proposed_Phase) // Start of Any Round
        {
            ResetList(List_Of_Slice_Ag1,Num_Of_Slice_Ag1);
            ResetList(List_Of_Slice_Ag2,Num_Of_Slice_Ag2);
            ResetList(List_Of_Slice_Ag3,Num_Of_Slice_Ag3);
            ...
            for (i = 0; i < Num_Of_Select; i++) {
                ...
                if (i == 0) {
                    ...
                    if (LearningRate(Q3, 3) < 9) MAS_Reward_Test += Envionment_Feedback_Agent2_Without_Ag1(
                                                     Slice2, Cell2);
        ...
        Cert_Episode = (Sum_Of_Cert / Cert_Counter);
        Print_Text("Certainty");
        cout << "The Certainty Of MAS in Episode  " << Episode_Number_Ag1 + Episode_Number_Ag2 << " Is: " <<
                Cert_Episode;
        /*********************************************************************
         *																 	  *
         *		                      Agent Efiiciency					      *
    ...
    while ((LearningRate(Q3, 3) < 9) && (Episode_Number_Ag3 + Episode_Number_Ag2 + Episode_Number_Ag1 <= Iteration))
    //Start Of Episode in Test Phase
    //for(int Iteration=1;Iteration<=100;Iteration++)
    {
        Print_Q_Table(Q1, 1);
            ...
            for (i = 0; i < Num_Of_Select; i++) {
                ...
                if (i == 0) {
                    ...
                    //    if(R3>=TS3)
                    {
                        ...
                        //Certain Parameter
                        Sum_Of_Cert += Certainty(Q3, Episode_Number_Ag1 + Episode_Number_Ag2 + Episode_Number_Ag3,
                                                 Slice3, Cell3);
                        Cert_Counter++;
                        //Certain Parameter													 	
                    }
        ...
        Cert_Episode = (Sum_Of_Cert / Cert_Counter);
        Print_Text("Certainty");
        cout << "The Certainty Of MAS in Episode  " << Episode_Number_Ag1 + Episode_Number_Ag2 + Episode_Number_Ag3 <<
                " Is: " << Cert_Episode;
        /*********************************************************************
         *																 	  *
         *		                      Agent Efiiciency					      *
