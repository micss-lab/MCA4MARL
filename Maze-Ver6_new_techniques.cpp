...
public:
    ...
    int chargingStationCount; // Number of charging stations in the subenvironment
...
void trainAgentWithStoppingCriterion(MazeNode *node, const vector<vector<int> > &maze, const int rows, const int cols,
    ...
    int minEpisodes = 500 + 10 * (endRow - startRow + 1);
    ...
    // Find neighboring leaves (assuming node is a leaf with parent containing all leaves)
    vector<MazeNode *> neighbors;
    if (node->parent) {
        for (MazeNode *sibling: node->parent->children) {
            if (sibling != node) {
    ...
    // Main training loop (with a stopping criterion)
    while (!converged && counter < maxEpisodes) {
        ...
        while (arrival == 0 && iteration < maxStepsPerEpisode) {
            ...
            // Check if action crosses boundary
            bool crossedBoundary = (x2 < startRow || x2 > endRow || y2 < startCol || y2 > endCol);
            if (crossedBoundary) {
                // Find neighbor containing (x2, y2)
                MazeNode *neighbor = nullptr;
                for (MazeNode *n: neighbors) {
                    if (x2 >= n->startRow && x2 <= n->endRow && y2 >= n->startCol && y2 <= n->endCol) {
                        neighbor = n;
                        break;
                    }
                }
                if (neighbor && neighbor->baselineSuccessRate >= 0) {
                    // Use neighbor's Q-values if trained
                    auto qValues = neighbor->getQValues(x2, y2);
                    if (!qValues.empty()) {
                        actionReward = *max_element(qValues.begin(), qValues.end()) * DISCOUNT_FACTOR;
                        // Discounted reward
                    } else {
                        actionReward = -0.1; // Small penalty if no Q-values
                    }
                } else {
                    // Fallback: Small penalty to encourage exploration
                    actionReward = -0.1;
                }
            }
        ...
        if (counter % 10 == 0 && counter >= minEpisodes) {
            ...
            if (policyUnchanged) {
                // Increment stable episodes
                stableEpisodes++;
                if (stableEpisodes >= patience) converged = true;
            } else {
                // Reset stable episodes
                stableEpisodes = 0;
...
/*************************************************************************/
tuple<bool, int, vector<pair<int, int> > > findValidPath(
    const vector<vector<int> > &maze, const MazeNode *root, const int rows, const int cols, const int startX,
    ...
    while (!toExplore.empty()) {
        ...
        // All q-values should be stored in the root node
        auto qValues = root->getQValues(x, y);
        int act = -1;
        if (!qValues.empty()) {
            act = distance(qValues.begin(), max_element(qValues.begin(), qValues.end()));
        }

        if (act == -1) {
            // Random action if no Q-values available
            vector<int> validActions;
            ...
            if (!validActions.empty()) {
                act = validActions[rand() % validActions.size()];
            } else {
                continue;
            }
        }
        ...
        if (newX >= 0 && newX < rows && newY >= 0 && newY < cols &&
            maze[newX][newY] != OBSTACLE && visited.find({newX, newY}) == visited.end()) {
...
/*************************************************************************/
tuple<double, double, double> testAgent(const vector<vector<int> > &maze, const MazeNode *root, const int rows,
                                        const int cols) {
    ...
    auto processChunk = [&](const size_t startIdx, const size_t endIdx) -> ThreadResult {
        ...
        for (size_t i = startIdx; i < endIdx && i < positions.size(); ++i) {
            ...
            auto [success, steps, path] = findValidPath(maze, root, rows, cols, x1, y1, maxStepsPerEpisode);
...
double computeNodeSuccessRate(const MazeNode *root, const MazeNode *node) {
    if (!root || !node || !root->maze || !node->qTable) {
        cerr << "Invalid input for computeNodeSuccessRate\n";
        return 0.0;
    }
    ...
    const auto &maze = *root->maze;
    const int rows = root->rows;
    const int cols = root->cols;
    const int maxSteps = rows + cols;
    ...
    for (int x = startRow; x <= endRow; ++x) {
        for (int y = startCol; y <= endCol; ++y) {
            ...
            auto [success, steps, path] = findValidPath(maze, root, rows, cols, x, y, maxSteps);
            if (success) {
                successfulPaths++;
            }
        }
...
//
//     const bool isInitialTraining = changedLeaves.empty();
//     constexpr double lowSuccessThreshold = 0.1; // Escalate if success < 0.1
//     constexpr double significantDropThreshold = 0.10; // Escalate if success drops significantly
//
//     cout << "\n--- Training Hierarchy Smart ---\n";
//
...
//     }
//
//     // Step 2: Decide which leaves to train
...
//                 const double baseline = leaf->baselineSuccessRate;
//                 const double newSuccessRate = computeNodeSuccessRate(root, leaf);
//                 cout << "Leaf: " << leaf->startRow << ", " << leaf->startCol
//                           << " - Baseline: " << baseline << ", New Success Rate: " << newSuccessRate << endl;
//                 if (baseline - newSuccessRate > getRetrainingThreshold(root->rows)) {
//                     leavesToRetrain.push_back(leaf);
//                     cout << "Leaf marked for retraining: " << leaf->startRow << ", " << leaf->startCol << endl;
//                 }
//             }
//         }
//     }
//
//     // Step 3: Train leaves and escalate recursively if needed
//     unordered_set<MazeNode*> trainedNodes; // Track all trained nodes to avoid duplication
//     if (!leavesToRetrain.empty()) {
//         // Train leaves
//         trainLeafNodesInParallel(root, leavesToRetrain, 1.0);
//         trainedNodes.insert(leavesToRetrain.begin(), leavesToRetrain.end());
//
//         // Check success rates and escalate
//         vector<MazeNode*> nodesToEscalate;
//         for (MazeNode* leaf : leavesToRetrain) {
//             const double baseline = leaf->baselineSuccessRate;
//             leaf->baselineSuccessRate = computeNodeSuccessRate(root, leaf);
//             const double newSuccessRate = leaf->baselineSuccessRate;
//             cout << "Updated baseline for leaf: " << leaf->startRow << ", " << leaf->startCol
//                       << " - New Baseline: " << newSuccessRate << endl;
//
//             // Escalate if success is low or retraining was ineffective
//             if ((newSuccessRate < lowSuccessThreshold || (baseline - newSuccessRate > significantDropThreshold)) &&
//                 leaf->parent) {
//                 nodesToEscalate.push_back(leaf->parent);
//                 cout << "Escalating leaf [" << leaf->startRow << "," << leaf->startCol << "] to parent\n";
//             }
//         }
//
//         // Recursive escalation
//         while (!nodesToEscalate.empty()) {
//             // Deduplicate nodes to train
//             unordered_set<MazeNode*> uniqueNodes(nodesToEscalate.begin(), nodesToEscalate.end());
//             vector<MazeNode*> nodesToTrain;
//             for (MazeNode* node : uniqueNodes) {
//                 if (trainedNodes.find(node) == trainedNodes.end()) {
//                     nodesToTrain.push_back(node);
//                 }
//             }
//
//             if (nodesToTrain.empty()) break;
//
//             // Train nodes
//             trainLeafNodesInParallel(root, nodesToTrain, 1.0);
//             trainedNodes.insert(nodesToTrain.begin(), nodesToTrain.end());
//
//             // Process trained nodes and escalate further if needed
//             vector<MazeNode*> nextNodesToEscalate;
//             for (MazeNode* node : nodesToTrain) {
//                 const double nodeSuccess = computeNodeSuccessRate(root, node);
//                 node->baselineSuccessRate = nodeSuccess;
//                 cout << "Trained node: " << node->startRow << ", " << node->startCol
//                           << " - Success: " << nodeSuccess << endl;
//
//                 // Propagate Q-values to children if successful
//                 if (nodeSuccess >= lowSuccessThreshold) {
//                     for (MazeNode* child : node->children) {
//                         propagateQTableDownwards(node);
//                         child->baselineSuccessRate = computeNodeSuccessRate(root, child);
//                         cout << "Propagated to child: " << child->startRow << ", " << child->startCol
//                                   << " - New Baseline: " << child->baselineSuccessRate << endl;
//                     }
//                 } else if (node->parent) {
//                     // Escalate to next level
//                     nextNodesToEscalate.push_back(node->parent);
//                     cout << "Escalating node [" << node->startRow << "," << node->startCol << "] to parent\n";
//                 }
//             }
//
//             nodesToEscalate = move(nextNodesToEscalate);
//         }
//     }
// }
...
void trainHierarchySmart(MazeNode *root, const vector<MazeNode *> &changedLeaves = {}) {
    ...
    if (!leavesToRetrain.empty()) {
        ...
        for (MazeNode *leaf: leavesToRetrain) {
            leaf->baselineSuccessRate = computeNodeSuccessRate(root, leaf); // Set new baseline after training
        }
    }
    ...
    while (!currentLevelNodes.empty()) {
        ...
        if (!nodesToTrain.empty()) {
            ...
            for (MazeNode *node: nodesToTrain) {
                node->baselineSuccessRate = computeNodeSuccessRate(root, node); // Set new baseline
            }
        }
...
void runFullExperiment() {
    ...
    for (int s = 0; s < sizes.size(); ++s) {
        ...
        for (int d = 0; d < difficulties.size(); ++d) {
            ...
            printMatrixInt(initialMaze, size, size, "Maze");
